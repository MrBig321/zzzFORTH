...
; _insw				INSW
;	( addr port -- )
; INSW to memaddr at EDI, from port in DX
;*********************************************
_insw:
			push edi
			POP_PS(edx)
...
.DCell		cmp DWORD [_state], COMPILE
			...
			COMPILE_CELL(edx)
.Again		jmp .Chk
.Found		mov ecx, eax					; EAX, ECX: dptolink 
			xor edx, edx
			mov dl, BYTE [eax+CELL_SIZE]	; EDX: flags|len
			and	dl, FLAGS_MASK				; EDX: flags 
			PUSH_PS(eax)
...
; _outsw				OUTSW
;	( addr port -- )
; OUTSW from memaddr at ESI, to port in DX
;*********************************************
_outsw:
			POP_PS(edx)
			POP_PS(eax)
...
_tick:
			...
			jmp .Back
.Find		call _find
			cmp DWORD [esi], 0		; is dptolinkptr zero?
			jz	.Err
 			call _to_body			; get dptoxt
...
; _w_plus			W+
;	( addr1 -- addr2 )
;	Adds the size of a word in bytes to addr1
;	giving addr2
;*********************************************
_w_plus:
			add DWORD [esi], (CELL_SIZE/2)
			ret
...
; Should move only if next item is non-zero !?
...
; _pause				PAUSE
;	( -- )
; Null word, just to have PAUSE available in 
; Non-Multitasking mode. This way, we don't 
; have to delete PAUSE from the FORTH-source codes.
;*********************************************
_pause:
			ret
...
_load_usb_fth:
			PUSH_PS(USB_BASE)
			mov	eax, [USB_SIZE_LOC]
			PUSH_PS(eax)
			call _loadm
			PUSH_PS(USBFS_BASE)
			mov	eax, [USBFS_SIZE_LOC]
			PUSH_PS(eax)
			call _loadm
			PUSH_PS(TRUE)
			ret
