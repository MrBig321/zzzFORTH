...
; d_l_shift				DLSHIFT
;	( d1 n -- d2 )
;	Return d2, the result of shifting d1 n bits 
...
; "-8." --> D2* --> D. --> -16: all higher bits of -8 is 1, so D. will print negative
_d_l_shift:
			POP_PS(ecx)
			POP_PS(edx)
			POP_PS(eax)
...
.Pos2		POP_PS(edx)
			POP_PS(ecx)
			mov [num1], ecx
			mov [num1+CELL_SIZE], edx
	; IN: num1,num2
	; OUT: res3 (triple cell)
			call umulDWithS
;push esi
;push ecx
;mov esi, res3
...
;pop esi
	...
	; OUT: res2 
			call udiv3CellByS 
			mov eax, [res2]
			mov edx, [res2+CELL_SIZE]	; we drop the highest 32-bits
			PUSH_PS(eax)
			PUSH_PS(edx)
			cmp edi, 1 
			jnz	.Back
...
umulDWithS:
			...
			clc
			mov DWORD [res3], 0
			mov DWORD [res3+CELL_SIZE], 0
			mov DWORD [res3+2*CELL_SIZE], 0
	; AB * C   (each letter corresponds to a 32-bit DWORD)
		; B*C
			mov eax, [num1]
			mov ebx, [num2]
			mul ebx
			mov [res3], eax
			mov [res3+CELL_SIZE], edx
		; A*C
			mov eax, [num1+CELL_SIZE]
			mov ebx, [num2]
			mul ebx
			add [res3+CELL_SIZE], eax		; adc !?
			adc [res3+2*CELL_SIZE], edx
			ret


; 1. we set EDX=0 and divide EDX:EAX by the 32-bit divisor (EDX will contain the remainder)
;    store the result in res+64
; 2. we divide EDX:EAX by the 32-bit divisor (EDX will contain the remainder)
;    store the result in res+32
; 3. we divide EDX:EAX by the 32-bit divisor (EDX will contain the remainder)
...
udiv3CellByS:
			mov ebx, [ddiv]
			xor edx, edx
			mov eax, [res3+2*CELL_SIZE]
			div	ebx
			mov [res2+2*CELL_SIZE], eax
			mov eax, [res3+CELL_SIZE]
			div	ebx
			mov [res2+CELL_SIZE], eax
			mov eax, [res3]
			div	ebx
			mov [res2], eax
...
; ECX: remainder
; used by _number_sign in core.asm
udiv64by32:
			push eax
			mov eax, edx
			xor edx, edx
...
; In:  EDX:EAX, EBX	
; Out: EDX:EAX
; used by _to_number in core.asm
umul64bitWith32bit:	
			push ecx
			push esi
			push ebp
