...
; Can read longfilename-entries too. Can only write shortfilename-entries (8.3 CAPITALS.EXT).
; Can cd into folders and back.
; Substitues unicode char by 'X', so files/folders can be read/cd'd.
...
%define USBFAT32_PATH_BUFF		0x4F0000
%define USBFAT32_NAME_BUFF		0x4F2000
%define USBFAT32_SECTOR_BUFF	0x4F3000						; for reading/writing sectors from/to FAT
%define USBFAT32_CLUSTER_BUFF	0x4F4000
...
.Copy		mov esi, USBFAT32_SECTOR_BUFF
			...
			mov [usbfat32_dir_clusters], ebx
			mov BYTE [USBFAT32_PATH_BUFF], USBFAT32_ROOTDIR_CHAR
			mov BYTE [USBFAT32_PATH_BUFF+1], 0
			...
			; 16-bit SCSI commands can transfer max this many clusters at a time
;			xor edx, edx
;			mov eax, 0xFFFF
;			xor ebx, ebx
;			mov bl, [usbfat32_sectors_per_cluster]
;			div ebx
			; Some pendrives don't work with AX below (e.g. my TOSHIBA 8GB, but it works with 0x9F0; my KINGSTON 16GB works with 0x100)
...
.CpLFN		call usbfat32_copy_lfn_entry
			jmp .Inc
.SFN		cmp BYTE [usbfat32_name_entries_num], 0
			jz	.PrSFN	
...
.ChkLong	cmp BYTE [usbfat32_long_list], 1
			jne	.Inc
.LongLis	mov al, [esi+USBFAT32_SFN_ATTRIB]
			and al, USBFAT32_ATTR_DIRECTORY_MASK
			jz	.File
			push ebx
			mov ebx, usbfat32_folderTxt
			call gstdio_draw_text
			pop ebx
.File:
...
usbfat32_cd:
			...
			call gstdio_draw_text
			jmp	.Back
.Inited		cmp BYTE [usbfat32_dirs_num], USBFAT32_MAX_DIR_DEPTH_NUM
			jge	.Back
...
; OUT: usbfat32_res (0 faliure)
; Can write SFN-filenames only (8.3 CAPITALS)
usbfat32_write:
			pushad
			mov DWORD [usbfat32_res], 0
			mov DWORD [usbfat32_curr_sector_num], -1
...
; Writes FSInfo sector to disk
;	Should be called before turning off the computer, if we wrote to the filesystem
; IN: -
; OUT: usbfat32_res (0 faliure)
...
.Root		mov ebx, [usbfat32_root_dir_cluster]
