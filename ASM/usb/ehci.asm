; NOTE! Search for !!?BUG?!! to see the bugs that were found during the translation of this code to FORTH
; EHCI
;********************************************
...
%define	EHCI_HEAP_INIT		0x170000		; from FORTH-Memory-Map(FMM) in forth/common.asm
%define	EHCI_HEAP_SIZE		0x30000			; from FORTH-Memory-Map(FMM) in forth/common.asm
...
.NoPPC		and DWORD [edx], ~EHCI_PORT_ENABLED
		...
		;;should we clear here USBStatus-portchange!? 
				; Check HCHalted
				mov ebx, [ehci_bar]
				add ebx, eax
...
.StoreLgId	mov ebx, [ehci_async_buff]
			...
			mov dx, [ebx]
			mov [ehci_langid], dx
			cmp	BYTE [usb_dd_manuf_idx], 0			; is there a manufacturer-string? ( offs 14)
			jz	.PrNoManuf
...
.PrSepar	mov ebx, ehci_SeparatorTxt
			call gstdio_draw_text
			cmp	BYTE [usb_dd_prod_idx], 0			; is there a product-string? ( offs 15)
			jnz	.ProdS
